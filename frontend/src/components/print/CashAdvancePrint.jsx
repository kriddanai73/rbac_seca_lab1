...
const CashAdvancePrint = ({ formData = {}, previewMode = false }) => {
...
const isOtherGroup =
  selectedGroup === 'Other' ||
  (!!selectedGroup && !departmentGroups.includes(selectedGroup));

const otherGroupText =
  formData.otherDepartmentGroup ||
  (isOtherGroup && selectedGroup !== 'Other' ? selectedGroup : '');

const isOtherDepartment =
  selectedDepartment === 'อื่นๆ' ||
  (!!selectedDepartment && !departmentTypes.includes(selectedDepartment));

const otherDepartmentText =
  formData.otherDepartmentType ||
  (isOtherDepartment && selectedDepartment !== 'อื่นๆ' ? selectedDepartment : '');
      ...
      <div style={{ marginBottom: '10px' }}>
        ...
        >
          {departmentGroups.map((g) => (
            ...
            >
              <span style={checkBoxStyle}>{selectedGroup === g ? '✓' : ''}</span>
              <span>{g}</span>
            </label>
          ))}
          ...
          >
            <span style={checkBoxStyle}>{isOtherGroup ? '✓' : ''}</span>
            <span>Other</span>
            <span
              style={{
                ...dottedInputStyle,
        ...
        >
          ...
          {departmentTypes.map((t) => (
            ...
            >
              <span style={checkBoxStyle}>{selectedDepartment === t ? '✓' : ''}</span>
              <span>{t}</span>
            </label>
          ))}
